"""Export data-fy26.json for the FY2026 (Jul 2025 - Jun 2026) dashboard page.
...
OUT_PATH = DEFAULT_WORKBOOK.parent / "data-fy26.json"
...
FY26_CAL = [(2025, 7), (2025, 8), (2025, 9), (2025, 10), (2025, 11), (2025, 12),
            (2026, 1), (2026, 2), (2026, 3), (2026, 4), (2026, 5), (2026, 6)]
FY25_CAL = [(2024, 7), (2024, 8), (2024, 9), (2024, 10), (2024, 11), (2024, 12),
...
def main() -> int:
    ...
    try:
        ...
        e_new = sum(hist.get((2024, m), {}).get("elec", 0.0) for m in range(7, 13))
        e_old = sum(hist.get((2023, m), {}).get("elec", 0.0) for m in range(7, 13))
        ...
        fy_cur = {
            ...
            "hedge": hedge,
            "forecastMeta": {
                "method": "FY2026 actuals vs a same-month-FY2025 forecast",
                "elecFactor": round(elec_factor, 3),
                "gasHedgePct": hedge,
                "baseWindow": "Jul 2023 - Jun 2025 billing history",
                "dataNote": ("June 2026 electric bills are partial until the next Avista "
                             "pull. Kintec gas Jan-Jun 2026 excludes the small Avista "
                             "transport charge (roughly 1-2% of the gas bill) pending "
                             "those invoices."),
            },
        }
        ...
        hdd_25 = [int(_num(ws_hdd.Cells(r, 3).Value)) for r in range(6, 18)]
        hdd_26 = [int(_num(ws_hdd.Cells(r, 4).Value)) for r in range(6, 18)]
        hdd_n = [int(_num(ws_hdd.Cells(r, 5).Value)) for r in range(6, 18)]
        hdd = {"fy26": hdd_25, "fy27": hdd_26, "normal": hdd_n}
